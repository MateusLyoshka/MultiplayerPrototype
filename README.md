...
* **Deterministic host selection.** When the 4 students connect, the server assigns peer ids 0,1,2,3 in connection order. The lobby UI (`client/prototype/scripts/multiplayer.gd`) shows the IP field and "Criar sala" button only to the client with `my_id % 4 == 0`; the other three see just the refresh button and the room list. The decision is taken inside the first `refresh_rooms` callback (gated by a `role_decided` flag) because the `PEER_ID` packet arrives after `_ready`, and `-1 % 4 == -1` in GDScript would misclassify everybody as a joiner if checked too early.
* **Wrong-IP handling.** `client/prototype/scripts/network_handler.gd::start_client` applies `server_peer.set_timeout(0, 5000, 10000)` right after `connect_to_host`. Without it, an unreachable IP would silently sit in the ENet handshake for ~30 s before giving up; with it, the client gets `EVENT_DISCONNECT`, drops back to the main menu, and `on_connection_error` fires so the menu can show a message.
...
* **Responsibility:** Owns the `ENetConnection` for its project and bridges raw packet events to gameplay code via Godot signals. After the 3-project split there is no `is_server` flag: each project (`server/`, `client/`, `professor/`) ships its own `network_handler.gd` that only implements the half it needs. The server version binds and emits `from_client_packet`; the client/professor versions connect outbound and emit `from_server_packet`.
* **Key Attributes:** Server side manages peer connections (`avaliable_peer_ids`, `peers_connected`). Client/professor side manages a single outbound `server_peer` plus `server_ip` and the `on_peer_connected` / `on_connection_error` signals consumed by the menu UIs.
...
* `QUIT_ROOM_REQUEST`: Notifies the server that the client is leaving the room or the server.
...
* `PEER_ID`: Connection confirmation, assigning the unique ID to the client machine. Sent immediately on `EVENT_CONNECT` so the client knows its `my_id` before any `REFRESH` arrives — the menu uses `my_id % 4 == 0` to decide host vs. joiner.
* `REFRESH`: Sends the updated lobby state (list of active rooms).
* `START_ROOM`: Reply to the room creator's `ROOM_REQUEST`. Carries the freshly allocated `room_id` and triggers the host's local transition into the world scene.
* `JOIN_ROOM`: Returns data about the room the player just entered (host IP, ephemeral port, current player ids).
* `QUIT_ROOM`: Forces or confirms the removal of a player from the room.
* `HAS_JOINED` / `HAS_QUITTED`: Pushed by the central server to the other clients in the same room when the roster changes (see `server/scripts/server_packet_handler.gd::join_request` and `quit_room_request`). The receiving client diffs against its local `spawned_ids` and spawns/despawns accordingly.
...
These are Layer-1 packets fired by the **central server** (not the in-game host) when the room roster changes.

### 🔔 `HAS_JOINED` (Server -> Clients in room)

* `HAS_JOINED`: Notifies the other clients in the room that one or more players joined. Carries the full updated `remote_ids` list so the receivers can call `sync_spawns` and converge.

### 🚪 `HAS_QUITTED` (Server -> Clients in room)

* `HAS_QUITTED`: Notifies the other clients that one or more players left. Same updated `remote_ids` list; `sync_spawns` despawns whoever disappeared.
...
6. **Translation**: Host receives `MINIGAME_GRADE` on Layer 1 inside `client/prototype/scripts/client_packet_handler.gd::packet_handler` (case `MINIGAME_GRADE`), decodes, and broadcasts `MINIGAME_GRADE_RESULT` on Layer 2. The host's own minigame scene sees the grade via a local loopback signal (`ClientPacketHandler.host_minigame_grade`) since ENet broadcasts do not loop back to the sender.
