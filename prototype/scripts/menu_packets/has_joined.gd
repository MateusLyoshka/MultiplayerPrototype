class_name HasJoinedPkt extends PacketTypeClass
...
static func create(_remote_ids: Array[int]) -> HasJoinedPkt:
	var packet = HasJoinedPkt.new(PACKET_TYPE.HAS_JOINED) # Certifique-se que o enum existe
	packet.remote_ids = _remote_ids
	return packet

static func create_from_data(data: PackedByteArray) -> HasJoinedPkt:
	var packet = HasJoinedPkt.new(PACKET_TYPE.HAS_JOINED)
	packet.decode(data)
	return packet
